...
def from_tcm_protein(tcm,
    ...
    chem_protein_links_list = []
    for p in protein['Ensembl_ID']:
        print(p)
        chem_protein_links_list.append(get.get_chem_protein_links('Ensembl_ID', [p], score=0))

    chem_protein_links = reduce(lambda df1, df2: pd.merge(df1, df2, on="HVCID", how="inner"), chem_protein_links_list)

    # 抽出需要移动的三列组成新的数据框
    new_df = chem_protein_links[['HVCID', 'Ensembl_ID_y', 'Combined_score_y']].copy()
    chem_protein_links.drop(['Ensembl_ID_y', 'Combined_score_y'], axis=1, inplace=True)
    chem_protein_links.rename(columns={'Ensembl_ID_x': 'Ensembl_ID', 'Combined_score_x': 'Combined_score'},
                              inplace=True)
    new_df.rename(columns={'Ensembl_ID_y': 'Ensembl_ID', 'Combined_score_y': 'Combined_score'}, inplace=True)

    chem_protein_links = chem_protein_links.reset_index(drop=True)
    chem_protein_links = pd.concat([chem_protein_links[['HVCID', 'Ensembl_ID', 'Combined_score']], new_df], axis=0)

    chem = np.intersect1d(chem_protein_links['HVCID'], tcm_chem_links['HVCID']).tolist()
...
if __name__ == '__main__':
    tcm, tcm_chem_links, chem, chem_protein_links, protein = temp_from_tcm_protein(tcm=['柴胡', '黄芩'],
